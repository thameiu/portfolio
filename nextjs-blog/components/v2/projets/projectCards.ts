import type { ProjectData, ProjectStoryBlock } from "./types";

type ProjectCardCopyItem = Partial<
  Pick<ProjectData, "description" | "details" | "linkText">
> & {
  story?: Record<string, Partial<ProjectStoryBlock>>;
};
type ProjectCardCopy = Record<string, ProjectCardCopyItem>;

/* ─── 4 projects ──────────────────────────────── */
const WEB_PROJECTS: ProjectData[] = [
    {
        id: "rgbast",
        title: "RGBast",
        fullTitle: "RGBast",
...
const OTHER_PROJECTS_BASE: ProjectData[] = [
    {
        id: "bunchofcards",
        title: "bunchofcards",
        fullTitle: "bunchofcards",
        titleSvg: "/bunchofcards/bunchofcards.svg",
        description: "classeur web de ma collection de cartes diverses.",
        details:
            "Expérience web rétro inspirée de Windows XP et du moteur Source · cartes scrollables orientées vers le curseur · visualisation 3D recto-verso.",
        techStack: ["React", "Vite", "TypeScript", "Three.js"],
        bgColor: "#FAF9FA",
        accentColor: "#C41BD4",
        isDark: false,
        iconType: "cubegrid",
        screenshots: [
            "/bunchofcards/bunchofcards-1.png",
            "/bunchofcards/bunchofcards-2.png",
            "/bunchofcards/bunchofcards-3.png",
            "/bunchofcards/bunchofcards-4.png",
            "/bunchofcards/bunchofcards-5.png",
            "/bunchofcards/bunchofcards-6.png",
        ],
        link: "https://cards.mathieu-hernandez.fr/",
        linkText: "voir bunchofcards",
        story: [
            {
                id: "bunchofcards-story-1",
                layout: "text",
                title: "Contexte",
                text: "Depuis de nombreuses années, je collectionne des **cartes** aléatoires, de restaurants où je mange, d’artistes que je rencontre, de boutiques que je visite… J’en ai tellement accumulé que mon portefeuille de l’époque commençait à se déchirer.<br/><br/>Je voulais faire quelque chose de ces cartes, et éventuellement, j’ai eu l’idée d’en faire un **classeur digital**, que j’ai appelé “**bunchofcards**”.",
            },
            {
                id: "bunchofcards-story-2",
                layout: "text",
                title: "Projet",
                text: "**bunchofcards** contient des dizaines de cartes que j’ai manuellement **scannées** et **éditées**, pour qu’elles soient correctement rognées et toutes à la même échelle. Il les présente via une liste **scrollable**, où toutes les cartes **s’orientent** constamment vers le curseur, ou alors sur une grille répétant les cartes à **l’infini**. Cliquer sur une carte permet de la visualiser en **3D, recto-verso**, et d’obtenir une description de celle-ci.<br/><br/>Sur ce projet j’ai voulu reproduire une esthétique rétro, proche de **Windows XP**, et s’inspirant également du moteur de jeu **Source** (à travers les fonds quadrillés rose et noir, et un easter egg).<br/><br/>Ces cartes que je collectionne représentent des instants ou **expériences** de ma vie. D’autres détails et fonctionnalités les utilisant sont présents sur le site.",
            },
        ],
    },
    {
...
export type ProjectCardGroups = {
  web: ProjectData[];
  others: ProjectData[];
};

function applyProjectCopy(project: ProjectData, copy?: ProjectCardCopy): ProjectData {
  const projectCopy = copy?.[project.id as keyof ProjectCardCopy];
  if (!projectCopy) return project;

  return {
    ...project,
    ...projectCopy,
    story: project.story?.map((block) => ({
      ...block,
      ...projectCopy.story?.[block.id],
    })),
  };
}

export const PROJECTS = WEB_PROJECTS;
export const OTHER_PROJECTS = OTHER_PROJECTS_BASE;

export function getProjectCardGroups(copy?: ProjectCardCopy): ProjectCardGroups {
  return {
    web: WEB_PROJECTS.map((project) => applyProjectCopy(project, copy)),
    others: OTHER_PROJECTS_BASE.map((project) => applyProjectCopy(project, copy)),
  };
}
