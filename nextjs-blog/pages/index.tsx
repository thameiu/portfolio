...
export default function Home() {
  ...
  // Wait for background to be ready before showing content
  useEffect(() => {
    if (backgroundReady) {
  ...
  return (
    <div className="min-h-screen p-2 flex flex-col justify-start items-center font-['sofia-pro-regular']">
      ...
      {/* Loading Screen */}
      {isLoading && (
        <div className="fixed inset-0 bg-[#192231] z-[9999] flex items-center justify-center">
          <div className="relative w-20 h-20 md:w-25 md:h-25 animate-pulse">
            <Image
      ...
      <div className={`w-full transition-opacity duration-500 ${isLoading ? 'opacity-0' : 'opacity-100'}`}>
        <BackgroundAnimation onReady={() => setBackgroundReady(true)} />
        <Header />
